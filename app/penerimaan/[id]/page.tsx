...
function FormPenerimaanContent() {
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto my-8 font-sans">
            <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-500 hover:text-emerald-600 font-bold mb-6 group transition-all"><ArrowLeft size={20} className="group-hover:-translate-x-1" /> Kembali ke Daftar</button>
            <div className="bg-white shadow-xl rounded-[2.5rem] border border-gray-100 overflow-hidden">
                <div className="bg-emerald-600 p-8 text-white flex items-center gap-5">
                    ...
                    <div><h1 className="text-2xl font-black uppercase">Penerimaan Perbaikan</h1><p className="text-emerald-100 text-xs font-medium uppercase mt-1">TAHUN {thn} | TAHAP F: PHP</p></div>
                </div>
                <div className="p-8">
                    {!docInfo ? <div className="p-4 bg-red-50 text-red-600 rounded-xl font-bold">⚠️ Data No. {id} Tahun {thn} Tidak Ditemukan.</div> : (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <select className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-emerald-100 font-bold text-gray-800" value={revisiKe} onChange={(e) => setRevisiKe(e.target.value)}>{[1,2,3,4,5].map(n => <option key={n} value={n}>{n}</option>)}</select></div>
                                <div><label className="block text-xs font-black mb-3 text-gray-500 uppercase">Tanggal Terima Berkas</label>
                                <input type="date" className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-emerald-100 font-bold text-gray-800" value={tanggal} onChange={(e) => setTanggal(e.target.value)} required /></div>
                            </div>
                            <div className="flex justify-end pt-8 border-t border-gray-50"><button type="submit" disabled={submitLoading} className="bg-emerald-600 hover:bg-emerald-700 text-white px-10 py-4 rounded-2xl font-black text-sm shadow-xl flex items-center gap-3">{submitLoading ? <Loader2 className="animate-spin" /> : <Save size={18} />} Simpan PHP</button></div>
                        </form>
                        </>
                    )}
                </div>
...
export default function PenerimaanDetailPage() {
    return <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-emerald-600 w-12 h-12" /></div>}><FormPenerimaanContent /></Suspense>;
}