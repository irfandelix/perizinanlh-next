        ...
        <div className="min-h-screen bg-slate-50 p-6 md:p-12 font-sans">
            <div className="mb-10 max-w-6xl mx-auto">
                <h1 className="text-3xl font-black text-slate-800 flex items-center gap-4 tracking-tight">
                    <div className="p-3 bg-emerald-600 rounded-2xl shadow-xl shadow-emerald-100 italic"><FileCheck className="text-white w-7 h-7" /></div>
                    PENERIMAAN PERBAIKAN (TAHAP F)
                </h1>
                <p className="text-slate-400 mt-3 ml-20 text-xs font-black uppercase tracking-[0.2em]">Pencatatan Berkas Hasil Perbaikan Konsultan (PHP)</p>
            </div>
                            ...
                            <table className="w-full text-left text-sm border-collapse">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {filteredData.map((doc) => (
                                        <tr key={doc._id} className="hover:bg-emerald-50/30 transition-all duration-200 group">
                                            <td className="p-6 text-center font-black text-slate-300 group-hover:text-emerald-600">{doc.noUrut}</td>
                                            <td className="p-6">
                                                <div className="flex flex-col gap-1.5">
                                                    <span className="inline-block w-fit px-2.5 py-0.5 rounded-md text-[9px] font-black bg-emerald-50 text-emerald-600 uppercase tracking-tighter border border-emerald-100">{doc.jenisDokumen}</span>
                                            ...
                                            <td className="p-6 text-center">
                                                {/* INI YANG DIGANTI: MENGGUNAKAN doc.noUrut */}
                                                <Link href={`/penerimaan/${doc.noUrut}?thn=${doc.tahun}`} className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md active:scale-95 ${doc.nomorPHP ? 'bg-slate-100 text-slate-400 hover:bg-slate-200' : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-100'}`}>
