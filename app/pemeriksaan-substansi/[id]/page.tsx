...
function FormPemeriksaanContent() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.post('/api/submit/d', { noUrut: parseInt(id), tahun: thn, tanggalPemeriksaan: tanggal });
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto my-8 font-sans">
            <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-500 hover:text-indigo-600 font-bold mb-6 group transition-all"><ArrowLeft size={20} className="group-hover:-translate-x-1" /> Kembali ke Daftar</button>
            <div className="bg-white shadow-xl rounded-[2.5rem] border border-gray-100 overflow-hidden">
                <div className="bg-indigo-600 p-8 text-white flex items-center gap-5">
                    ...
                    <div><h1 className="text-2xl font-black uppercase">Pemeriksaan Substansi</h1><p className="text-indigo-100 text-xs font-medium uppercase mt-1">TAHUN {thn} | TAHAP D</p></div>
                </div>
                <div className="p-8">
                    {!docInfo ? <div className="p-4 bg-red-50 text-red-600 rounded-xl font-bold">⚠️ Data No. {id} Tahun {thn} Tidak Ditemukan.</div> : (
                        <>
                        <div className="bg-indigo-50 border border-indigo-100 rounded-[2rem] p-6 mb-8 flex gap-4"><Info className="text-indigo-500 mt-1" />
                            <div className="grid grid-cols-2 gap-4 w-full">
                                ...
                                <div><span className="text-[10px] font-black text-gray-400 uppercase">No Urut / Tahun</span><p className="font-black text-indigo-600 mt-1">{docInfo.noUrut} / {docInfo.tahun}</p></div>
                            </div>
                        </div>
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="max-w-md"><label className="block text-xs font-black mb-3 text-gray-500 uppercase">Tanggal BA Pemeriksaan</label>
                            <input type="date" className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-100 font-bold text-gray-800" value={tanggal} onChange={(e) => setTanggal(e.target.value)} required /></div>
                            <div className="flex justify-end pt-8 border-t border-gray-50"><button type="submit" disabled={submitLoading} className="bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-4 rounded-2xl font-black text-sm shadow-xl flex items-center gap-3">{submitLoading ? <Loader2 className="animate-spin" /> : <Save size={18} />} Simpan BA</button></div>
                        </form>
                        </>
                    )}
                </div>
...
export default function PemeriksaanDetailPage() {
    return <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-indigo-600 w-12 h-12" /></div>}><FormPemeriksaanContent /></Suspense>;
}