...
import { MapPin, CheckCircle, Clock, Loader2, AlertCircle, ChevronRight, ExternalLink } from 'lucide-react'; 
...
interface Dokumen {
    ...
    fileTahapC?: string; // TAMBAHAN: Properti untuk menyimpan URL Google Drive BA Verlap
}
        ...
        <div className="min-h-screen bg-slate-50 p-6 md:p-12 font-sans">
            <div className="mb-10 max-w-6xl mx-auto">
                <h1 className="text-3xl font-black text-slate-800 flex items-center gap-4 tracking-tight">
                    <div className="p-3 bg-amber-500 rounded-2xl shadow-xl shadow-amber-100 italic">
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center p-24 text-amber-500">
                        ...
                        <div className="flex flex-col items-center justify-center p-24 text-slate-300">
                            ...
                            <p className="font-black uppercase text-xs tracking-widest text-center">Kosong di tahun {selectedYear}</p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-left text-sm border-collapse">
                                <thead className="bg-slate-50/50 text-slate-400 font-black uppercase text-[10px] tracking-[0.2em] border-b border-slate-50">
                                    <tr>
                                        ...
                                        <th className="p-6 text-center">Status Verlap</th>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {filteredData.map((doc) => (
                                        <tr key={doc._id} className="hover:bg-amber-50/30 transition-all duration-200 group">
                                            <td className="p-6 text-center font-black text-slate-300 group-hover:text-amber-600 bg-gray-50/30">{doc.noUrut}</td>
                                            <td className="p-6">
                                                <div className="flex flex-col gap-1.5">
                                                    <span className="inline-block w-fit px-2.5 py-0.5 rounded-md text-[9px] font-black bg-amber-50 text-amber-600 uppercase tracking-tighter border border-amber-100">
                                                        {doc.jenisDokumen}
                                                    </span>
                                                    <div className="font-black text-slate-800 uppercase line-clamp-1 leading-tight tracking-tight">
                                                        {doc.namaKegiatan || "(TANPA JUDUL)"}
                                                    </div>
                                                    <div className="font-mono text-[10px] text-slate-400 tracking-tighter uppercase">
                                            ...
                                            <td className="p-6 text-center">
                                                {doc.nomorBAVerlap ? (
                                                    ...
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black bg-red-50 text-red-600 border border-red-100 uppercase tracking-widest animate-pulse">
                                                        <Clock className="w-3.5 h-3.5" /> Belum Verlap
                                                    </span>
                                                )}
                                            </td>
                                            <td className="p-6">
                                                <div className="flex items-center justify-center gap-2">
                                                    ...
                                                    {/* TOMBOL LIHAT FILE DRIVE (Dinamis berdasarkan data) */}
                                                    {doc.fileTahapC && (
                                                        <a 
                                                            href={doc.fileTahapC} 
                                                            target="_blank" 
                                                            ...
                                                            title="Buka BA Verlap di Google Drive"
                                                        >
                                                    ...
                                                    >
                                                        {doc.nomorBAVerlap ? 'Detail' : 'Periksa'}
