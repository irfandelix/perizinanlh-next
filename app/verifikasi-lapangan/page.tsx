...
export default function FormVerifikasi() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.post('/api/submit/c', { noUrut: docInfo.noUrut, tahun: docInfo.tahun, tanggalVerlap: tanggal });
        ...
        <div className="p-6 max-w-4xl mx-auto my-8 font-sans">
            ...
            <div className="bg-white shadow-xl rounded-[2.5rem] border border-gray-100 overflow-hidden">
                <div className="bg-amber-600 p-8 text-white flex items-center gap-5">
                    <div className="p-4 bg-white/20 rounded-2xl backdrop-blur-md"><MapPin size={28} /></div>
                    <div>
                        <h1 className="text-2xl font-black uppercase tracking-tight">Verifikasi Lapangan</h1>
                        <p className="text-amber-100 text-xs font-medium uppercase tracking-widest mt-1">TAHUN {thn} | TAHAP C</p>
                    </div>
                </div>
                <div className="p-8">
                    {docInfo && (
                        <div className="bg-amber-50 border border-amber-100 rounded-[2rem] p-6 mb-8 flex gap-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 w-full">
                                ...
                                <div><span className="text-[10px] font-black text-gray-400 uppercase">No Urut / Tahun</span><p className="font-black text-amber-600">{docInfo.noUrut} / {docInfo.tahun}</p></div>
                            </div>
                        </div>
                    )}
                    <form onSubmit={handleSubmit} className="space-y-8">
                        <div className="max-w-md">
                            <label className="block text-xs font-black mb-3 text-gray-500 uppercase">Tanggal Verifikasi Lapangan</label>
                            <input type="date" className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-amber-100 font-bold text-gray-800" value={tanggal} onChange={(e) => setTanggal(e.target.value)} required />
                        </div>
                        <div className="flex justify-end pt-8 border-t border-gray-50">
                            <button type="submit" disabled={submitLoading} className="bg-amber-600 text-white px-10 py-4 rounded-2xl font-black text-sm shadow-xl flex items-center gap-3">
                                {submitLoading ? <Loader2 className="animate-spin" /> : <Save size={18} />} Simpan BA Verlap
                            </button>
                        </div>
                    </form>
                </div>
