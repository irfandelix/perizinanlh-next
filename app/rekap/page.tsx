...
const DynamicTableContent = dynamic(() => Promise.resolve(TableContent), { ssr: false, loading: () => (<div className="rekap-table-wrapper p-8 text-center text-gray-500 font-medium animate-pulse">Memuat tampilan tabel...</div>) });
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">            
            <div className="flex justify-between items-end mb-6">
                ...
                <button onClick={handleDownloadExcel} className='bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-bold shadow-md transition-all flex items-center gap-2 disabled:opacity-50' disabled={loading || filteredData.length === 0}>
            ...
            {availableYears.length > 0 && (
                <div className="flex gap-2 border-b-2 border-emerald-100 mb-6 overflow-x-auto pt-2 pl-2">
                    {availableYears.map((year) => (
                        <button key={year} onClick={() => setSelectedYear(year)} className={`px-8 py-3 rounded-t-xl font-bold text-sm transition-all whitespace-nowrap ${selectedYear === year ? 'bg-emerald-600 text-white shadow-[0_-4px_10px_rgba(0,0,0,0.1)] transform -translate-y-1' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-200 hover:text-emerald-900'}`}>Tahun {year}</button>
                    ))}
                </div>
            )}
